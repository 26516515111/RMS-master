...
void LoginDia::on_chPwdEdit_clicked()
{
    ...
    if(!query1->exec("select * from users")){
        qDebug() << "查询失败：" ;
    }

    while(query1->next()){
        QString account2 =  ui->accountEdit->text();
        QString account1 = query1->value(0).toString();
        ...
        if((account2 == account1) && (password1 == oriPwd)){
            ...
            query1->prepare("UPDATE users SET password = :newpassword WHERE account = :account");

            query1->bindValue(":newpassword",newPwd);

            query1->bindValue(":account",account2);
